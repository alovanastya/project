...
namespace Prac2 {
	...
	public:
		...
		MyFormAdd(Medicine* medicines, int* last, System::Windows::Forms::DataGridView^ dataGridView1)
		{
			InitializeComponent();
...
public: System::Void buttonOk_Click(System::Object^ sender, System::EventArgs^ e){
	...
	if (medicines[*last].name.length() == 0) medicines[*last].name = "Название";
	if (medicines[*last].maker.length() == 0) medicines[*last].maker = "Производитель";
	if (medicines[*last].category.length() == 0) medicines[*last].category = "Категория";
	if (medicines[*last].form.length() == 0) medicines[*last].form = "Форма";
